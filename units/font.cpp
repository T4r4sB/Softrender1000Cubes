...
{	
	...
	void OutSymbol (const tbal::Bitmap &b, char c, Point pos, Fixed scale, Fixed width, tbal::Color color)
	{
		...
		for (int i=0; i<s.header.count; ++i)
		{
			...
			if (p1==p2)
			{
				if (p1==p3)
				{
					tbal::Line(b, pos + p1*scale, pos + p2*scale, width, color, false, false);
				}	else
				{
					const Point sc = pos+(p1+p3)*scale*fxHalf;
					const Fixed r = (p3-p1).Length()*scale*fxHalf;
					tbal::Arc(b, sc, r, width, Point( 1,0), Point(-1,0), color, true, true);
					tbal::Arc(b, sc, r, width, Point(-1,0), Point( 1,0), color, true, true);
				}
			} else
			{
				Fixed ss = (p2-p1)^(p3-p1);
				if (ss == Fixed(0))
					tbal::Line(b, pos + p1*scale, pos + p2*scale, width, color, false, false);
				else 
				{				
					const Point sc = (((p1-p2)*(p3*p3)+(p2-p3)*(p1*p1)+(p3-p1)*(p2*p2))/(ss*Fixed(2))).Rot90();		
					...
					if ((a1^a3)>=fx0)
					{
						if ((a3^a2)>=fx0)
						{
							tbal::Arc(b, scs, srs, width, a1, a3, color, false, true);
							tbal::Arc(b, scs, srs, width, a3, a2, color, true,  false);
						} else if ((a1^a2)>=fx0)
						{
							tbal::Arc(b, scs, srs, width, a2, -a2, color, false, true);
							tbal::Arc(b, scs, srs, width, -a2, a1, color, true , false);
						} else
						{
							tbal::Arc(b, scs, srs, width, a1, -a1, color, false, true);
							tbal::Arc(b, scs, srs, width, -a1, a2, color, true , false);
						}
					} else 
					{
						if ((a3^a2)<=Fixed(0))
						{
							tbal::Arc(b, scs, srs, width, a2, a3, color, false, true);
							tbal::Arc(b, scs, srs, width, a3, a1, color, true , false);
						} else if ((a1^a2)<=Fixed(0))
						{
							tbal::Arc(b, scs, srs, width, a1, -a2, color, false, true);
							tbal::Arc(b, scs, srs, width, -a2, a2, color, true , false);
						} else
						{
							tbal::Arc(b, scs, srs, width, a2, -a1, color, false, true);
							tbal::Arc(b, scs, srs, width, -a1, a1, color, true , false);
						}
					} 					
				}
			}	
	...
	void Font::Init (Fixed ascale, Fixed awidth)
	{
		if (ascale == scale || awidth == width) return;
		scale = ascale;
		width = awidth;
